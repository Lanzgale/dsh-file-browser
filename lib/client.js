window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = `
html {
  --fe-panel-width: 360px;
  --fe-panel-shift: calc(var(--fe-panel-width) + 12px);
}
...
.fe-resize:hover { background: var(--fe-accent); opacity: .35; }
.fe-drag-capture {
...
.fe-iconbtn:hover { background: var(--fe-bg-2); color: var(--fe-fg); }
.fe-iconbtn-on { color: var(--fe-accent); }
...
.fe-editor-head {
  display: flex; align-items: center; gap: 2px; /* 与面板标题栏 .fe-header 的 gap 一致 */
  padding: 7px 8px; flex: none; /* 与面板标题栏 .fe-header 同高(7px) */
  border-bottom: 1px solid var(--fe-border);
  color: var(--fe-fg-2); font-size: 13px; /* 与面板基础字号一致(13px) */
}
.fe-editor-name { font-weight: 600; color: var(--fe-fg); flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.fe-btn {
...
.fe-toggle:hover { background: var(--fe-bg-2); color: var(--fe-fg); }
.fe-toggle-on { color: var(--fe-accent); }
.fe-hl { tab-size: 4; }
...
`;
		...
		const api = {
			list: (path) => fetch('/plugins/file-browser/list?path=' + encodeURIComponent(path)).then((r) => r.json()),
		...
		// 右侧面板互斥协议:打开自己时广播,其他面板插件(仓库浏览器等)
		...
		const setOpen = (value) => {
			const next = !!value;
			if (next && !store.open) {
				window.dispatchEvent(new CustomEvent(PANEL_EVENT, { detail: 'file-browser' }));
			}
			store.open = next;
			emit();
		};
		...
		const iconPaths = {
			...
			folderOpen: 'M19 20H4a2 2 0 0 1-2-2V6c0-1.11.89-2 2-2h6l2 2h7a2 2 0 0 1 2 2H4v10l2.14-8h17.07l-2.28 8.5c-.23.87-1.01 1.5-1.93 1.5',
			...
			formatAnnotationPlus: 'M8.5,7H10.5L16,21H13.6L12.5,18H6.3L5.2,21H3L8.5,7M7.1,16H11.9L9.5,9.7L7.1,16M22,5V7H19V10H17V7H14V5H17V2H19V5H22Z',
			formatAnnotationMinus: 'M10.5,7H8.5L3,21H5.2L6.3,18H12.5L13.6,21H16L10.5,7M7.1,16L9.5,9.7L11.9,16H7.1M22,7H14V5H22V7Z',
		};
		...
		const ToggleButton = () => {
			...
			return react.createElement('button', {
				...
				'aria-label': '文件浏览器',
				onClick: toggleOpen,
			}, react.createElement(Icon, { name: 'folderOpen', size: 15 }));
		};
			...
			if (isDir && expanded) {
				if (children) {
					for (const child of children) {
						...
						nodes.push(react.createElement(TreeNode, { key: child.path, entry: child, depth: props.depth + 1, tree, showHidden: props.showHidden, onToggle: props.onToggle, onSelect: props.onSelect, onOpen: props.onOpen }));
					}
		...
		const ExplorerPanel = (props) => {
			...
			const [dark, setDark] = react.useState(() => {
				try { return localStorage.getItem('fe-theme') !== 'light' } catch (e) { return true }
			});
			const toggleDark = () => {
				setDark((v) => {
					const next = !v;
					try { localStorage.setItem('fe-theme', next ? 'dark' : 'light') } catch (e) {}
					return next;
				});
			};
			const [drag, setDrag] = react.useState(null);
			const [fontScale, setFontScale] = react.useState(() => {
				try { const v = parseFloat(localStorage.getItem('fe-font-scale')); return isFinite(v) && v >= 0.6 && v <= 1.8 ? v : 1 } catch (e) { return 1 }
			});
			const changeFont = (delta) => {
				setFontScale((v) => {
					const next = Math.min(1.8, Math.max(0.6, Math.round((v + delta) * 10) / 10));
					try { localStorage.setItem('fe-font-scale', String(next)) } catch (e) {}
					return next;
				});
			};
			...
			react.useEffect(() => {
				...
				if (s.open && !heroPage) root.setAttribute('data-fe-panel-open', '');
			...
			const onResizeStart = (kind, e) => {
				e.preventDefault();
				setDrag({ kind, startX: e.clientX, startWidth: s.width });
			};
			const onResizeMove = (e) => {
				if (!drag) return;
				store.width = Math.max(360, Math.min(720, drag.startWidth + (drag.startX - e.clientX)));
				emit();
			};
				...
				if (tree.expanded.has(tree.rootPath)) {
					...
					if (children) {
						for (const child of children) {
							...
							rows.push(react.createElement(TreeNode, { key: child.path, entry: child, depth: 1, tree, showHidden, onToggle: toggleDir, onSelect: selectFile, onOpen: (e, toggle) => openFile(e, false, toggle) }));
						}
			...
			const renderPreview = () => {
				...
				const head = react.createElement('div', { className: 'fe-editor-head' },
					...
					isMd && editor.state === 'ready' && !editor.editing
						? react.createElement('button', { className: 'fe-iconbtn', title: editor.preview ? '源码' : '预览', onClick: () => setEditor((e) => ({ ...e, preview: !e.preview })) }, react.createElement(Icon, { name: editor.preview ? 'fileCodeOutline' : 'fileDocumentOutline', size: 14 }))
						: null,
					react.createElement('button', { className: 'fe-iconbtn', title: '缩小字号', onClick: () => changeFont(-0.1) }, react.createElement(Icon, { name: 'formatAnnotationMinus', size: 14 })),
					react.createElement('button', { className: 'fe-iconbtn', title: '放大字号', onClick: () => changeFont(0.1) }, react.createElement(Icon, { name: 'formatAnnotationPlus', size: 14 })),
					...
					react.createElement('button', { className: 'fe-iconbtn', title: '返回文件目录', onClick: () => { setEditor(null); setStatus(null) } }, react.createElement(Icon, { name: 'arrowRight', size: 15 })),
				);
				let body = null;
			...
			const treePanel = react.createElement('div', { className: 'fe-panel' + (dark ? ' fe-theme-dark' : ''), style: { width: s.width + 'px', '--fe-preview-scale': fontScale } },
				react.createElement('div', { className: 'fe-resize', title: '拖动调整宽度', onPointerDown: (e) => onResizeStart('tree', e) }),
				editor ? null : react.createElement('div', { className: 'fe-header' },
					react.createElement('span', { className: 'fe-title' }, '文件浏览器'),
					react.createElement('button', { className: 'fe-iconbtn' + (dark ? ' fe-iconbtn-on' : ''), title: dark ? '切换到浅色' : '切换到深色', onClick: toggleDark }, react.createElement(Icon, { name: dark ? 'sun' : 'moon', size: 14 })),
					react.createElement('button', { className: 'fe-iconbtn' + (showHidden ? ' fe-iconbtn-on' : ''), title: showHidden ? '隐藏隐藏文件' : '显示隐藏文件', onClick: () => setShowHidden((v) => !v) }, react.createElement(Icon, { name: 'eye', size: 14 })),
				...
				editor ? renderPreview() : react.createElement(react.Fragment, null,
					status ? react.createElement('div', { className: 'fe-status ' + (status.ok ? 'fe-status-ok' : 'fe-status-err') }, status.text) : null,
					react.createElement('div', { className: 'fe-tree' }, renderTree()),
				),
			);
		...
		function apply(ctx) {
			...
			const onPanelOpen = (e) => {
				if (e && e.detail && e.detail !== 'file-browser') setOpen(false);
			};
			window.addEventListener(PANEL_EVENT, onPanelOpen);
			ctx.effect(() => () => { window.removeEventListener(PANEL_EVENT, onPanelOpen) }, 'file-browser: panel mutex');

			const slots = ctx.get('slots');
			if (slots === undefined) return;
			slots.inject('shell.overlay', () => slots.register(
				{ name: 'shell.overlay', id: "file-browser", order: 90, label: '文件浏览器' },
				(props) => react.createElement(ExplorerPanel, props),
			));
			slots.inject('conversation.session.header.actions', () => slots.register(
				{ name: 'conversation.session.header.actions', id: 'file-browser-toggle', order: 30, label: '文件浏览器' },
