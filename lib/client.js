...
.fe-search {
  ...
  border: 1px solid var(--fe-border); border-radius: 5px;
  color: var(--fe-fg); font-size: 12px; outline: none;
}
.fe-search:focus { border-color: var(--fe-accent); }
.fe-search::placeholder { color: var(--fe-fg-2); }
.fe-search-state {
...
.fe-editor-head {
  ...
  border-bottom: 1px solid var(--fe-border);
  color: var(--fe-fg-2); font-size: 12px;
}
...
`;
		...
		const api = {
			...
			search: (root, q) => fetch('/plugins/file-browser/search?root=' + encodeURIComponent(root) + '&q=' + encodeURIComponent(q)).then((r) => r.json()),
		...
		const iconPaths = {
			...
			textDec: 'M0.99 19h2.42l1.27-3.58h5.65L11.59 19h2.42L8.75 5h-2.5L0.99 19zm4.42-5.61l2.03-5.79h.12l2.03 5.79H5.41zM20 11v2h-8v-2h8z',
			textInc: 'M0.99 19h2.42l1.27-3.58h5.65L11.59 19h2.42L8.75 5h-2.5L0.99 19zm4.42-5.61l2.03-5.79h.12l2.03 5.79H5.41zM20 11h3v2h-3v3h-2v-3h-3v-2h3V8h2v3z',
		};
		...
		const ExplorerPanel = (props) => {
			...
			const refresh = () => {
				...
				// 保留展开状态,清空缓存后重新加载所有已展开目录
				const expanded = new Set(tree.expanded);
				if (!expanded.has(root)) expanded.add(root); // 根目录始终加载
				setTree({ rootPath: root, rootName: name, expanded, cache: new Map(), loading: new Set(expanded), selected: tree.selected, errors: {} });
			...
			const renderPreview = () => {
				...
				const head = react.createElement('div', { className: 'fe-editor-head' },
					...
					isMd && editor.state === 'ready' && !editor.editing
						...
						: null,
					react.createElement('button', { className: 'fe-iconbtn', title: '缩小字号', onClick: () => changeFont(-0.1) }, react.createElement(Icon, { name: 'textDec', size: 14 })),
					react.createElement('button', { className: 'fe-iconbtn', title: '放大字号', onClick: () => changeFont(0.1) }, react.createElement(Icon, { name: 'textInc', size: 14 })),
